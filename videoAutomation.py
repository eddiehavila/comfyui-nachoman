...
Single-chunk processing nodes for simplified HUMO music video generation.
Processes one 3.88-second chunk at a time.

Key nodes:
1. NACHOMAN_FullSongAnalyzerV4 - Analyzes entire song once, generates all prompts (cached)
   - Includes frames_per_chunk as separate INT output for video combine nodes

2. NACHOMAN_ChunkIndexController - Manages which chunk to process (auto vs manual)
   - Auto mode: Counts existing video_chunk_*.done marker files
   - Manual mode: Uses provided index

3. NACHOMAN_GetPromptByIndex - Extracts single prompt from full prompts by index

4. NACHOMAN_LoadSingleAudioChunk - Loads one audio chunk by index with precise timing

5. NACHOMAN_SaveVideoChunkWithIndex - Saves video chunk with proper naming
...
6. NACHOMAN_AutoQueueController - Manages auto-queueing and completion
   - Queues next chunk automatically when chunks remain
   - Signals final combine when all chunks are complete

7. NACHOMAN_CombineAllChunks - Combines all video chunks into final video
...
try:
    ...
    print("[NACHOMAN V4] Warning: Whisper (Transformers) not available. Transcription will be disabled.")
...
# =============================================================================
# Node 1: NACHOMAN_FullSongAnalyzerV4
# =============================================================================

class NACHOMAN_FullSongAnalyzerV4:
    """
    ...
    CATEGORY = "NACHOMAN/V4 Single Chunk"
    ...
    def analyze_song(
        ...
        print("\n" + "="*80)
        print("[NACHOMAN V4] 🎵 Full Song Analyzer - Starting analysis...")
        print("[NACHOMAN V4] Parameters:")
        ...
        # Extract audio data
        print("[NACHOMAN V4] Step 1/5: Extracting audio data...")
        try:
            waveform = audio["waveform"]  # shape: [channels, samples]
            sample_rate = audio["sample_rate"]
            print(f"[NACHOMAN V4]   ✓ Audio shape: {waveform.shape}, Sample rate: {sample_rate}Hz")
        except Exception as e:
            raise ValueError(f"Invalid audio input: {e}")

        # Calculate audio metadata
        print("[NACHOMAN V4] Step 2/5: Calculating audio metadata...")
        ...
        print(f"[NACHOMAN V4]   ✓ Duration: {audio_duration:.2f}s | Hash: {audio_hash}")

        # Calculate total chunks needed
        print("[NACHOMAN V4] Step 3/5: Calculating chunk breakdown...")
        fps = 25
        ...
        if abs(scene_duration_seconds - expected_duration) > 0.01:
            print(f"[NACHOMAN V4]   ⚠️ scene_duration_seconds ({scene_duration_seconds}s) differs from default ({expected_duration}s)")
            print(f"[NACHOMAN V4]   Recalculating frames_per_chunk to match requested duration...")
            frames_per_chunk = int(scene_duration_seconds * fps)
            print(f"[NACHOMAN V4]   New frames_per_chunk: {frames_per_chunk}")

        seconds_per_chunk = frames_per_chunk / fps

        total_chunks = math.ceil(audio_duration / seconds_per_chunk)

        print(f"[NACHOMAN V4]   ✓ FPS: {fps}")
        ...
        print("[NACHOMAN V4] Step 4/5: Transcription phase...")
        if enable_full_transcription and WHISPER_AVAILABLE:
            print(f"[NACHOMAN V4] 🎤 Transcribing full audio (language: {language})...")

            try:
                # Load Whisper model (using Transformers)
                device = "cuda" if torch.cuda.is_available() else "cpu"
                print(f"[NACHOMAN V4]   Loading Whisper model on {device}...")
                model_name = "openai/whisper-large-v3"
                processor = WhisperProcessor.from_pretrained(model_name)
                model = WhisperForConditionalGeneration.from_pretrained(model_name).to(device).eval()
                print("[NACHOMAN V4]   ✓ Model loaded")

                # Convert audio to format Whisper expects
                # Whisper expects mono audio at 16kHz
                print("[NACHOMAN V4]   Converting audio to mono @ 16kHz...")
                print(f"[NACHOMAN V4]   Original waveform shape: {waveform.shape}, sample_rate: {sample_rate}Hz")
                target_sr = 16000
                ...
                print(f"[NACHOMAN V4]   After mono conversion: {waveform_mono.shape}")

                # Resample if needed
                if sample_rate != target_sr:
                    print(f"[NACHOMAN V4]   Resampling from {sample_rate}Hz to {target_sr}Hz...")
                    waveform_mono = torchaudio.functional.resample(waveform_mono, sample_rate, target_sr)
                    print(f"[NACHOMAN V4]   After resampling: {waveform_mono.shape}")

                print(f"[NACHOMAN V4]   ✓ Audio prepared: {waveform_mono.shape} ({waveform_mono.size(0)} samples = {waveform_mono.size(0)/target_sr:.2f}s)")

                # Transcribe full audio in chunks (30 second max)
                print("[NACHOMAN V4]   Starting full audio transcription...")
                ...
                for i, chunk in enumerate(audio_chunks):
                    print(f"[NACHOMAN V4]     Processing audio segment {i+1}/{len(audio_chunks)}...", end="\r")
                ...
                print(f"\n[NACHOMAN V4]   ✅ Full transcription complete ({len(full_lyrics)} chars)")
                ...
                for idx in range(total_chunks):
                    try:
                        ...
                        print(f"[NACHOMAN V4]     Chunk {idx+1}/{total_chunks}: samples {trans_start}-{trans_end} ({chunk_audio.size(0)} samples)")
                        ...
                        if chunk_audio.size(0) > 0:
                            ...
                            print(f"[NACHOMAN V4]       → Result: \"{preview}...\" ({len(transcription.split())} words)")
                        else:
                            print(f"[NACHOMAN V4]       → Skipped (no audio)")
                            chunk_transcriptions.append("")

                    except Exception as e:
                        print(f"[NACHOMAN V4]       ⚠️ Error transcribing chunk {idx+1}: {e}")
                        chunk_transcriptions.append("")

                print(f"\n[NACHOMAN V4]   ✅ Transcribed {len(chunk_transcriptions)} individual chunks (with context)")
                ...
                if chunk_transcriptions:
                    ...
                    if word_counts:
                        ...
                        print(f"[NACHOMAN V4]   📊 Average words per chunk: {avg_words:.1f} (includes overlap context)")
                        print(f"[NACHOMAN V4]   📊 Word count range: {min(word_counts)}-{max(word_counts)} words")

                        # Show preview of first 3 chunks to verify split
                        print(f"[NACHOMAN V4]   📝 Chunk lyrics preview:")
                        for i in range(min(3, len(chunk_transcriptions))):
                            ...
                            print(f"[NACHOMAN V4]      Chunk {i}: \"{preview}...\" ({word_count} words)")

            except Exception as e:
                print(f"[NACHOMAN V4] ⚠️ Transcription failed: {e}")
                import traceback
                traceback.print_exc()
                full_lyrics = f"[Transcription failed: {str(e)}]"
                chunk_transcriptions = [""] * total_chunks
        else:
            print("[NACHOMAN V4] ℹ️ Transcription disabled or Whisper not available")
        ...
        print("[NACHOMAN V4] Step 5/5: Prompt generation phase...")
        print(f"[NACHOMAN V4] 🎨 Generating {total_chunks} scene prompts...")

        # Process placeholder words for empty/minimal lyrics
        print("[NACHOMAN V4]   Processing placeholder words for empty chunks...")
        print(f"[NACHOMAN V4]   Min lyric words threshold: {min_lyric_words}")
        print(f"[NACHOMAN V4]   Placeholder options: {placeholder_words}")
        ...
        for idx, transcription in enumerate(chunk_transcriptions):
            ...
            if word_count < min_lyric_words:
                ...
                if idx < 5:  # Show first few replacements
                    print(f"[NACHOMAN V4]     Chunk {idx}: Empty ({word_count} words) → \"{placeholder}\"")
            else:
                # Keep original transcription
                processed_chunk_lyrics.append(transcription)

        if empty_chunk_count > 0:
            print(f"[NACHOMAN V4]   ✓ Replaced {empty_chunk_count}/{total_chunks} empty chunks with placeholders")
        else:
            print(f"[NACHOMAN V4]   ✓ All chunks have sufficient lyrics (no placeholders needed)")

        # Create pipe-separated lyrics for prompt generation
        print("[NACHOMAN V4]   Preparing lyrics for prompt generation...")
        if processed_chunk_lyrics:
            pipe_separated_lyrics = " | ".join(processed_chunk_lyrics)
        else:
            pipe_separated_lyrics = full_lyrics
        print(f"[NACHOMAN V4]   ✓ Lyrics prepared ({len(pipe_separated_lyrics)} chars)")

        # Build LLM instructions using template with placeholder replacement
        print("[NACHOMAN V4]   Building LLM instructions from template...")
        ...
        print(f"[NACHOMAN V4]   ✓ Instructions built from template ({len(llm_instructions)} chars)")
        print(f"[NACHOMAN V4]   Template placeholders replaced: {list(llm_params.keys())}")

        # Generate WAN2.1 compatible prompts
        print("[NACHOMAN V4]   Generating WAN2.1 compatible prompts...")
        print(f"[NACHOMAN V4]   Using seed {seed} for deterministic randomness")
        ...
        print(f"[NACHOMAN V4]   Visual options parsed:")
        print(f"[NACHOMAN V4]     - Environments: {len(env_opts)} options")
        print(f"[NACHOMAN V4]     - Lighting: {len(light_opts)} options")
        print(f"[NACHOMAN V4]     - Camera motions: {len(camera_opts)} options")
        print(f"[NACHOMAN V4]     - Physical interactions: {len(phys_opts)} options")
        print(f"[NACHOMAN V4]     - Facial expressions: {len(face_opts)} options")
        print(f"[NACHOMAN V4]     - Shot types: {len(shot_opts)} options")
        print(f"[NACHOMAN V4]     - Outfits: {len(outfit_opts)} options")
        ...
        for idx in range(total_chunks):
            ...
            if (idx + 1) % 10 == 0 or idx == total_chunks - 1:
                print(f"[NACHOMAN V4]     Generated {idx+1}/{total_chunks} prompts...")

        print(f"[NACHOMAN V4]   ✅ Generated {len(generated_prompts)} WAN2.1 prompts")

        # Show preview of first 3 prompts to verify variety
        print(f"[NACHOMAN V4]   📝 Prompt preview (first 3):")
        for i in range(min(3, len(generated_prompts))):
            preview = generated_prompts[i][:120] if generated_prompts[i] else "[empty]"
            print(f"[NACHOMAN V4]      Prompt {i}: \"{preview}...\"")
            # Show word count
            word_count = len(generated_prompts[i].split())
            print(f"[NACHOMAN V4]               ({word_count} words)")
        ...
        print(f"[NACHOMAN V4] Audio metadata created:")
        ...
        print("="*80)
        print(f"[NACHOMAN V4] ✅ Analysis complete! Ready for chunked processing.")
        print(f"[NACHOMAN V4] Summary:")
        ...
        print(f"[NACHOMAN V4] Data structure:")
...
# =============================================================================
# Node 2: NACHOMAN_ChunkIndexController
# =============================================================================

class NACHOMAN_ChunkIndexController:
    """
    ...
    def get_index(self, full_prompts, output_folder, auto_index, manual_index):
        ...
        print("\n" + "="*80)
        print("[NACHOMAN V4] 🎯 Chunk Index Controller - Determining chunk to process...")
        print(f"[NACHOMAN V4] Mode: {'AUTO' if auto_index else 'MANUAL'}")
        print(f"[NACHOMAN V4] Output folder (raw): {output_folder}")
        ...
        print(f"[NACHOMAN V4] Output folder (absolute): {output_folder_abs}")

        # Detect folder change
        folder_changed = False
        if NACHOMAN_ChunkIndexController._last_folder is None:
            print(f"[NACHOMAN V4] First run - initializing folder tracking")
            NACHOMAN_ChunkIndexController._last_folder = output_folder_abs
        elif NACHOMAN_ChunkIndexController._last_folder != output_folder_abs:
            folder_changed = True
            print(f"[NACHOMAN V4] 🔄 FOLDER CHANGE DETECTED!")
            print(f"[NACHOMAN V4]   Previous: {NACHOMAN_ChunkIndexController._last_folder}")
            print(f"[NACHOMAN V4]   Current:  {output_folder_abs}")
            print(f"[NACHOMAN V4]   → Resetting index to 0")
            NACHOMAN_ChunkIndexController._last_folder = output_folder_abs

        total_chunks = full_prompts.get("total_chunks", 0)
        print(f"[NACHOMAN V4] Total chunks in song: {total_chunks}")

        if auto_index:
            # Auto-detect index by counting existing files
            if folder_changed:
                # Folder changed, start from 0 (will count files to verify)
                print("[NACHOMAN V4] Folder changed - starting fresh from index 0")
                current_index = 0
                ...
                if completed_count > 0:
                    ...
                    print(f"[NACHOMAN V4]   Found {completed_count} existing chunks in new folder")
            else:
                # Normal auto-detection
                print("[NACHOMAN V4] Scanning output folder for completed chunks...")
                current_index = self._count_completed_chunks(output_folder_abs)
                print(f"[NACHOMAN V4]   ✓ Found {current_index} completed chunks")
            mode_str = "AUTO"
        else:
            # Use manual index
            current_index = manual_index
            mode_str = "MANUAL"
            print(f"[NACHOMAN V4]   Using manual index: {current_index}")

        # Calculate remaining chunks
        remaining = total_chunks - current_index - 1
        print(f"[NACHOMAN V4] Current index: {current_index}")
        print(f"[NACHOMAN V4] Remaining after this: {remaining}")
        ...
        if current_index >= total_chunks:
            ...
            print(f"[NACHOMAN V4] Status: ALL CHUNKS COMPLETE!")
        else:
            status_message = f"[{mode_str}] Processing chunk {current_index + 1}/{total_chunks} ({remaining} remaining)"
            print(f"[NACHOMAN V4] Status: Processing chunk {current_index + 1}/{total_chunks}")
            print(f"[NACHOMAN V4] Auto-queue next: {should_auto_queue}")
    ...
    def _count_completed_chunks(self, folder_path):
        ...
        if not os.path.isdir(folder_path):
            print(f"[NACHOMAN V4]   ℹ️ Folder doesn't exist yet: {folder_path}")
            return 0

        try:
            print(f"[NACHOMAN V4]   Scanning folder: {folder_path}")

            # List all files for debugging
            all_files = os.listdir(folder_path)
            print(f"[NACHOMAN V4]   Total files in folder: {len(all_files)}")
            if len(all_files) > 0 and len(all_files) <= 20:
                print(f"[NACHOMAN V4]   All files: {', '.join(sorted(all_files))}")
            ...
            print(f"[NACHOMAN V4]   .done marker files found: {len(marker_files)}")
            ...
            if len(marker_files) == 0:
                ...
                if len(marker_files) > 0:
                    print(f"[NACHOMAN V4]   Using .mp4 files for counting (legacy mode)")
                    print(f"[NACHOMAN V4]   .mp4 chunk files found: {len(marker_files)}")

            count = len(marker_files)
            if count > 0:
                print(f"[NACHOMAN V4]   Marker files: {', '.join(sorted(marker_files)[:5])}" + ("..." if len(marker_files) > 5 else ""))
            else:
                print(f"[NACHOMAN V4]   No marker files found - will start from index 0")

            print(f"[NACHOMAN V4]   Total completed chunks: {count}")
            print(f"[NACHOMAN V4]   Next chunk index will be: {count}")
            return count
        except Exception as e:
            print(f"[NACHOMAN V4] ⚠️ Error counting chunks: {e}")
            import traceback
            traceback.print_exc()
            return 0
...
# =============================================================================
# Node 3: NACHOMAN_GetPromptByIndex
# =============================================================================

class NACHOMAN_GetPromptByIndex:
    """
    ...
    def get_prompt(self, full_prompts, index):
        ...
        print("\n" + "="*80)
        print("[NACHOMAN V4] 📝 Get Prompt By Index - Extracting prompt...")
        print(f"[NACHOMAN V4] Requested index: {index}")
        ...
        print(f"[NACHOMAN V4] Available prompts: {len(prompts)}")

        if index < 0 or index >= len(prompts):
            error_msg = f"❌ Index {index} out of range (0-{len(prompts)-1})"
            print(f"[NACHOMAN V4] {error_msg}")
            print("="*80 + "\n")
            return (error_msg, "", "", "")
        ...
        print(f"[NACHOMAN V4]   ✓ Extracted prompt for chunk #{index}")
        print(f"[NACHOMAN V4]   ✓ Prompt length: {len(prompt)} chars")

        if transcription:
            word_count = len(transcription.split())
            print(f"[NACHOMAN V4]   ✓ Original lyrics: {word_count} words")
            print(f"[NACHOMAN V4]   ✓ Original preview: \"{transcription[:60]}...\"")
        else:
            print(f"[NACHOMAN V4]   ✓ No original transcription")

        if processed != transcription:
            print(f"[NACHOMAN V4]   ✓ Processed lyrics (placeholder): \"{processed}\"")
        else:
            print(f"[NACHOMAN V4]   ✓ Processed lyrics: Same as original")

        if llm_instructions:
            print(f"[NACHOMAN V4]   ✓ LLM instructions: {len(llm_instructions)} chars")

        print(f"[NACHOMAN V4] Prompt preview: \"{prompt[:100]}...\"")
        print("="*80 + "\n")
...
# =============================================================================
# Node 4: NACHOMAN_LoadSingleAudioChunk
# =============================================================================

class NACHOMAN_LoadSingleAudioChunk:
    """
    ...
    def load_chunk(self, audio, index, audio_meta):
        ...
        print("\n" + "="*80)
        print("[NACHOMAN V4] 🎵 Load Single Audio Chunk - Extracting audio...")
        print(f"[NACHOMAN V4] Requested chunk index: {index}")

        try:
            waveform = audio["waveform"]  # shape: [channels, samples]
            sample_rate = audio["sample_rate"]
            print(f"[NACHOMAN V4]   ✓ Audio loaded: {waveform.shape} @ {sample_rate}Hz")
        except Exception as e:
            raise ValueError(f"Invalid audio input: {e}")

        # Get chunk parameters from audio_meta
        print(f"[NACHOMAN V4] Audio metadata received:")
        ...
        if meta_sample_rate != sample_rate:
            print(f"[NACHOMAN V4] ⚠️ SAMPLE RATE MISMATCH DETECTED!")
            print(f"[NACHOMAN V4]   audio_meta sample_rate: {meta_sample_rate} Hz")
            print(f"[NACHOMAN V4]   Actual audio sample_rate: {sample_rate} Hz")
            print(f"[NACHOMAN V4]   This will cause incorrect chunk durations!")
            print(f"[NACHOMAN V4]   Recalculating samples_per_chunk using ACTUAL sample rate...")
        ...
        num_samples = waveform.shape[-1]
        print(f"[NACHOMAN V4] Sample range: {start_sample} to {end_sample} (total: {num_samples})")

        # Extract chunk
        if start_sample >= num_samples:
            # Beyond audio end - create silence
            print("[NACHOMAN V4] ⚠️ Beyond audio end - creating silence")
            print(f"[NACHOMAN V4]   waveform shape: {waveform.shape}")
            ...
            print(f"[NACHOMAN V4]   silence shape: {chunk_waveform.shape}")
            actual_samples = 0
        else:
            ...
            print(f"[NACHOMAN V4] Extracted {actual_samples} samples")

            # Pad with silence if needed
            if actual_samples < samples_per_chunk:
                padding = samples_per_chunk - actual_samples
                print(f"[NACHOMAN V4] Padding with {padding} samples of silence")
                print(f"[NACHOMAN V4]   chunk_waveform shape: {chunk_waveform.shape}")
                ...
                print(f"[NACHOMAN V4]   silence shape: {silence.shape}")
                chunk_waveform = torch.cat([chunk_waveform, silence], dim=-1)
                print(f"[NACHOMAN V4]   padded shape: {chunk_waveform.shape}")
...
# =============================================================================
# Node 5: NACHOMAN_SaveVideoChunkWithIndex
# =============================================================================

class NACHOMAN_SaveVideoChunkWithIndex:
    """
    ...
    def save_chunk(self, index, output_folder, images=None, audio_chunk=None, vhs_filenames=None, fps=25.0):
        ...
        print("\n" + "="*80)
        print("[NACHOMAN V4] 💾 Save Video Chunk - Starting save process...")
        print(f"[NACHOMAN V4] Chunk index: {index}")
        print(f"[NACHOMAN V4] Output folder (raw): {output_folder}")
        print(f"[NACHOMAN V4] Mode: {'VHS' if vhs_filenames is not None else 'Direct'}")
        ...
        if not os.path.isabs(output_folder):
            ...
            print(f"[NACHOMAN V4] Resolved to absolute path: {output_folder}")

        # Create output folder if it doesn't exist
        print(f"[NACHOMAN V4] Ensuring output folder exists...")
        os.makedirs(output_folder, exist_ok=True)
        print(f"[NACHOMAN V4]   ✓ Folder ready: {output_folder}")
        ...
        print(f"[NACHOMAN V4] Target files:")
        ...
        try:
            ...
            if images is not None and audio_chunk is not None:
                print(f"[NACHOMAN V4] Using Direct mode (ffmpeg)...")
                ...
                print(f"[NACHOMAN V4]   Converting {images.shape[0]} frames to video...")
                ...
                with imageio.get_writer(temp_video_path, fps=fps, codec='libx264', quality=8) as writer:
                    for i, frame in enumerate(frames_np):
                        ...
                        if (i + 1) % 25 == 0:
                            print(f"[NACHOMAN V4]     Progress: {i+1}/{len(frames_np)} frames", end="\r")
                print(f"\n[NACHOMAN V4]   ✓ Frames written to temp video")

                # Save audio
                print(f"[NACHOMAN V4]   Saving audio...")
                waveform = audio_chunk["waveform"]
                sample_rate = audio_chunk["sample_rate"]

                print(f"[NACHOMAN V4]     Original waveform shape: {waveform.shape}")

                # Use soundfile to save WAV (more reliable than torchaudio)
                # torchaudio has a bug that causes std::length_error
                try:
                    print(f"[NACHOMAN V4]     Attempting to save with soundfile library...")
                    ...
                    print(f"[NACHOMAN V4]     After squeeze: {save_waveform.shape}")
                    ...
                    print(f"[NACHOMAN V4]     Audio shape for soundfile: {audio_np.shape} (samples, channels)")

                    sf.write(temp_audio_path, audio_np, sample_rate, subtype='PCM_16')
                    print(f"[NACHOMAN V4]   ✓ Audio saved to temp file (soundfile)")

                except ImportError:
                    print(f"[NACHOMAN V4]     soundfile not available, falling back to torchaudio")
                    try:
                        ...
                        print(f"[NACHOMAN V4]   ✓ Audio saved to temp file (torchaudio)")
                    except Exception as e:
                        print(f"[NACHOMAN V4]     ❌ torchaudio.save failed: {e}")
                        import traceback
                        traceback.print_exc()
                        raise
                except Exception as e:
                    print(f"[NACHOMAN V4]     ❌ soundfile.write failed: {e}")
                    import traceback
                    traceback.print_exc()
                    raise

                # Combine video + audio with ffmpeg
                print(f"[NACHOMAN V4]   Combining video and audio with ffmpeg...")
                ffmpeg_cmd = [
                    'ffmpeg',
                    '-y',  # Overwrite output
                ...
                if result.returncode != 0:
                    print(f"[NACHOMAN V4] FFmpeg stderr: {result.stderr}")
                    raise RuntimeError(f"FFmpeg failed: {result.stderr}")

                print(f"[NACHOMAN V4]   ✓ Video+audio combined successfully")

                # Clean up temp files
                os.remove(temp_video_path)
                os.remove(temp_audio_path)
                print(f"[NACHOMAN V4]   ✓ Temp files cleaned up")

            # MODE 2: VHS mode - move/rename VHS-saved file (FALLBACK)
            else:
                print(f"[NACHOMAN V4] Direct mode inputs not available, trying VHS mode...")
                ...
                if vhs_files:
                    print(f"[NACHOMAN V4] Using VHS mode (fallback)...")

                    source_path = vhs_files[0]  # Most recent
                    print(f"[NACHOMAN V4]   Found VHS file: {os.path.basename(source_path)}")
                    print(f"[NACHOMAN V4]   Moving to: {video_path}")

                    # Move and rename the main video file (with audio)
                    shutil.move(source_path, video_path)
                    print(f"[NACHOMAN V4]   ✓ Moved: {os.path.basename(source_path)} → {os.path.basename(video_path)}")
                    ...
                    if os.path.exists(source_noaudio):
                        ...
                        print(f"[NACHOMAN V4]   ✓ Removed: {os.path.basename(source_noaudio)} (non-audio duplicate)")

                    print(f"[NACHOMAN V4]   ✓ Video chunk saved successfully (VHS mode)")
                    ...
                    print(f"[NACHOMAN V4] ❌ {error_msg}")
                    print(f"[NACHOMAN V4] Suggestions:")
                    print(f"[NACHOMAN V4]   1. Connect images and audio_chunk inputs for Direct mode, OR")
                    print(f"[NACHOMAN V4]   2. Ensure VHS_VideoCombine runs before this node and saves files")
                    raise ValueError(error_msg)

            # Create marker file for counting (in both modes)
            print(f"[NACHOMAN V4] Creating marker file...")
...
# =============================================================================
# Node 6: NACHOMAN_AutoQueueController
# =============================================================================

class NACHOMAN_AutoQueueController:
    """
    ...
    def control_queue(self, trigger, should_auto_queue, remaining, current_index, output_folder):
        ...
        print("\n" + "="*80)
        print("[NACHOMAN V4] 🔄 Auto Queue Controller - Managing queue...")
        print(f"[NACHOMAN V4] Current chunk: {current_index}")
        print(f"[NACHOMAN V4] Remaining chunks: {remaining}")
        print(f"[NACHOMAN V4] Should auto-queue: {should_auto_queue}")
        print(f"[NACHOMAN V4] Output folder (raw): {output_folder}")
        ...
        # Detect folder change
        folder_changed = False
        if NACHOMAN_AutoQueueController._last_folder is None:
            print(f"[NACHOMAN V4] First run - initializing folder tracking")
            NACHOMAN_AutoQueueController._last_folder = output_folder_abs
        elif NACHOMAN_AutoQueueController._last_folder != output_folder_abs:
            folder_changed = True
            print(f"[NACHOMAN V4] 📁 Folder changed detected!")
            print(f"[NACHOMAN V4]   Previous: {NACHOMAN_AutoQueueController._last_folder}")
            print(f"[NACHOMAN V4]   Current:  {output_folder_abs}")
            print(f"[NACHOMAN V4]   🔄 Resetting trigger_combine to False")
            NACHOMAN_AutoQueueController._last_folder = output_folder_abs
        ...
        if should_auto_queue and remaining > 0:
            ...
            print(f"[NACHOMAN V4] Queueing next chunk ({current_index + 1})...")
            try:
                PromptServer.instance.send_sync("impact-add-queue", {})
                status = f"✅ Chunk {current_index} done. Queued chunk {current_index + 1} ({remaining} more after this)"
                print(f"[NACHOMAN V4]   ✓ Successfully queued next chunk")
                print(f"[NACHOMAN V4] Status: {status}")
            except Exception as e:
                status = f"⚠️ Failed to queue next chunk: {e}"
                print(f"[NACHOMAN V4] ❌ Queue failed: {e}")
        else:
            if remaining <= 0 and should_auto_queue and not folder_changed:
                ...
                print(f"[NACHOMAN V4] 🏁 All chunks have been processed!")
                print(f"[NACHOMAN V4] 🎬 Triggering final video combine...")
            elif folder_changed:
                status = f"📁 Folder changed - trigger_combine reset to False"
                print(f"[NACHOMAN V4] 📁 Folder changed - not triggering combine")
            else:
                status = f"⏸️ Auto-queue disabled (manual mode)"
                print(f"[NACHOMAN V4] ⏸️ Auto-queue disabled (manual mode)")
            print(f"[NACHOMAN V4] Status: {status}")
...
# =============================================================================
# Node 7: NACHOMAN_CombineAllChunks
# =============================================================================

class NACHOMAN_CombineAllChunks:
    """
    ...
    def combine_chunks(self, trigger, output_folder, final_video_name, total_chunks):
        ...
        print("\n" + "="*80)
        print("[NACHOMAN V4] 🎬 Combine All Chunks - Checking trigger...")
        print(f"[NACHOMAN V4] Trigger value: {trigger} (type: {type(trigger)})")
        ...
        if trigger is not True:
            print(f"[NACHOMAN V4] ℹ️ Trigger is not True - skipping combine")
            print(f"[NACHOMAN V4] (Connect AutoQueueController's trigger_combine output to this node's trigger input)")
            print("="*80 + "\n")
            return ("", False)

        print("[NACHOMAN V4] ✓ Trigger is True - proceeding with combine")
        print(f"[NACHOMAN V4] Output folder (raw): {output_folder}")
        print(f"[NACHOMAN V4] Expected chunks: {total_chunks}")
        print(f"[NACHOMAN V4] Final video name: {final_video_name}")
        ...
        if "final" not in final_video_name.lower():
            ...
            print(f"[NACHOMAN V4] Added 'final' prefix: {final_video_name}")

        # Ensure filename has .mp4 extension
        if not final_video_name.endswith('.mp4'):
            final_video_name = f"{final_video_name}.mp4"
            print(f"[NACHOMAN V4] Added .mp4 extension: {final_video_name}")
        ...
        try:
            ...
            if total_chunks > 0 and len(chunk_files) < total_chunks:
                print(f"[NACHOMAN V4] ⚠️ Warning: Expected {total_chunks} chunks but found {len(chunk_files)}")
                print(f"[NACHOMAN V4]   Proceeding with available chunks...")
            ...
            print(f"[NACHOMAN V4] Creating concat file...")
            ...
            concat_file.close()
            print(f"[NACHOMAN V4]   ✓ Concat file created: {concat_file_path}")

            # Prepare output path
            final_video_path = os.path.join(output_folder, final_video_name)

            # Run ffmpeg concat
            print(f"[NACHOMAN V4] Running ffmpeg concat...")
            print(f"[NACHOMAN V4]   Combining {len(chunk_files)} chunks...")
            ...
            print(f"[NACHOMAN V4]   ✓ Concat file cleaned up")

            # Verify output
            if os.path.exists(final_video_path):
                file_size_mb = os.path.getsize(final_video_path) / (1024 * 1024)
                print(f"[NACHOMAN V4] ✅ Final video created successfully!")
...
NODE_CLASS_MAPPINGS = {
    "NACHOMAN_FullSongAnalyzerV4": NACHOMAN_FullSongAnalyzerV4,
    "NACHOMAN_ChunkIndexController": NACHOMAN_ChunkIndexController,
    "NACHOMAN_GetPromptByIndex": NACHOMAN_GetPromptByIndex,
    ...
    "NACHOMAN_AutoQueueController": NACHOMAN_AutoQueueController,
    "NACHOMAN_CombineAllChunks": NACHOMAN_CombineAllChunks,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "NACHOMAN_FullSongAnalyzerV4": "🎵 NACHOMAN Full Song Analyzer V4",
    "NACHOMAN_ChunkIndexController": "🎯 NACHOMAN Chunk Index Controller",
    "NACHOMAN_GetPromptByIndex": "📝 NACHOMAN Get Prompt By Index",
    "NACHOMAN_LoadSingleAudioChunk": "🎵 NACHOMAN Load Single Audio Chunk",
    "NACHOMAN_SaveVideoChunkWithIndex": "💾 NACHOMAN Save Video Chunk",
    "NACHOMAN_AutoQueueController": "🔄 NACHOMAN Auto Queue Controller",
    "NACHOMAN_CombineAllChunks": "🎬 NACHOMAN Combine All Chunks",
}
